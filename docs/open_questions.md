...
## Q13. 交差行は低レベル MCP からしか出ない（4 プロジェクトで測定）

- **測定**（`scripts/ablation.py`、7 木）:

  | プロジェクト | 登録形 | trig | 交差行 |
  |---|---|---|---|
  | mcp-server-git | 低レベル `@server.call_tool()` | **traced** | **12** |
  | AutoGPT | `@command(名前, 説明, {スキーマ})` | assumed | 0 |
  | PraisonAI | `tools=[f, g]` の裸の関数 | assumed | 0 |
  | langroid | `ToolMessage` 派生 + 同名ハンドラ | assumed | 0 |

- **交差行 12 行 / 1 プロジェクト。§3 の「3 行が異なるプロジェクト由来」を
  満たさないので不合格。**
- **AutoGPT が `assumed` になる理由は正しい。** ディスパッチは
  `agent.command_registry.get_command(command_name)` で、`self.commands` は
  実行時に `register(cmd)` で埋まる辞書である。候補集合が静的なリテラルでは
  ないので Def 4 の `OPAQUE(dynamic_registry)` に当たる。**推定で候補を
  埋めてはならない。**
- **したがって §3 の「統一の証拠」を満たすには、低レベル MCP サーバを
  もう 2 プロジェクト足す必要がある。** 高レベル登録形の母集団をいくら
  足しても交差行は増えない（SELECT 座標が発火しないから）。
- **決める時点**: C1（wild run）の母集団設計。低レベル経路のサーバを
  意図的に含めるか、それとも §3 の「統一の証拠は逸話的」／「三部品として
  報告する」分岐に入るかを、標本抽出の**前に**決めること。
  抽出後に決めると事後選択になる。
