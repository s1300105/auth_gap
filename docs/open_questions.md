...
## O23. 矛盾関係の表（`docs/contradiction_matrix.md`）で「決めが要る」と残した 5 マス（+ #6 / #7）— **解決（D55 / D56）。学生が原理を選び、原理から導いた表で判定する**
...
* ~~原理 1〜4 は学生の選択待ち~~ → **学生の決定（2026-09-24）: 1-i b / 1-ii b / 2 a / 3 a / 4 a**
  （研究の観点を優先）。§7 の表で実装した（D56、run13）。主指標（D1 + D2 の矛）165、不明 181。
  感度分析は `docs/contradiction_by_decl.md`。残った問題は O30（テストファイルへの末尾名の連鎖）、
  O32（相対 URL の宛先）、O33（主体 MODEL は「選べる」ではなく「流れ込む」）。
...
## O29. DB 効果の受け手型が解決できず、SQL 呼び出しの大半を落としている（誤 clear）— **解決（D50 / D51 / D54）。未解決率 90.0% → 17.6%（深さ 4）。残り 532 件の 85.2% は深さ 4 でも上限に当たるユニットの中**
...
→ **学生の決定は (a)「4 に上げる」（2026-09-24）。D54 / 逸脱 #17。** run11 は深さ 4 の
感度分析の run と全木一致（集合の表示順を除く）。未解決率 **17.6%**、CONTRADICTION **146**。
残りの `db_unresolved` 532 件のうち 453 件（85.2%）は、深さ 4 でも上限に当たるユニットの中にある（深さ 5 なら未解決率 14.2%）。**これ以上は上げない**（学生の決定）ので、限界として報告する。

## O30. 末尾名の解決がテストファイルの定義に結びつき、段数ぶん連鎖する — **解決（D57、§9.1。学生の指示で修正）**

- **状況**: 受け手の型で裏付けられない呼び出しは、D17 改訂で「末尾名で降りて確度に
  `opaque(unresolved)` を合流する」ことにしている。**降りる先の候補にテストファイルの定義も入る。**
  xagent では `logger.error(...)` が `tests/core/test_execution_scope.py:67` の関数内クラス
  `_Recorder.error` に結ばれ、その中の `append` がさらに末尾名で結ばれた。深さ 4 で増えた
  効果の経路の **26%（1,651 件中 430 件）** がこの 1 本の連鎖で、**テストファイルの中にある効果が
  0 → 348（深さ 4）→ 389（深さ 5）。** praisonai（較正木 A9）でも
  `_smtp_list_emails` の `…logout()` が `WhatsAppWebAdapter.logout` に結ばれた。
- **誤りの向き**: **誤警報**（本来無い効果を足す）。ただし確度は `opaque(unresolved)` なので、
  行は `UNKNOWN` に出て CONTRADICTION には数えない（D53 の 108 件の検証では、末尾名が原因の
  誤検出は `page.route` の 1 件）。
- **要る判断**: (a) テストファイル（`tests/` / `test_*.py`）の定義を末尾名の候補から外す。
  **ただし母集団にはテストファイル内でツールを定義しているものがある**（mcp-linkedin 9 件）。
  (b) 関数の中で定義されたクラスを末尾名の候補から外す。(c) 変えない（`opaque(unresolved)` で
  区別できているとして限界に書く）。**(a) / (b) は解決を狭める変更なので、消える経路を
  `compare_scans.py` で 1 件ずつ見る。**

- **追記（2026-09-24、D56）**: 判定原理を入れた後、**CONTRADICTION にも効くようになった**。
  run13 で D4（idempotent）の `fs_append` 5 件と D2 の `db_model_sql` 1 件が、この連鎖の先の
  テストファイル内の効果（`tests/e2e/test_channel_delivery_recovery.py:47`、
  `task_execution_event_store.py:97`）から立っている。**誤検出。**

## O31. 定数の `None` / `False` を渡した分岐を区別しない（経路非感応）— **解決（D58 (a)、`860aa93`）**

- **状況**: 深さ 4 で増えた CONTRADICTION の誤検出 5 件のうち 4 件がこれ。
  `write_text(None, text)` の先頭 `if not path: return None`（yt-scribe 3 件）、
  `sync_models(settings, force=False)` の `if force and target_dir.exists(): shutil.rmtree(...)`
  （tg-note 1 件）。**呼び出し元が定数を渡しているので、sink に届く経路は実行されない。**
- **誤りの向き**: **誤警報**（CONTRADICTION）。深さ 3 では上限の手前で止まって見えなかった。
- **要る判断**: (a) 仮引数に定数が束縛された呼び出しで、その仮引数だけを見る分岐の
  到達不能な側を刈る（**文脈感応の要約になるので費用と規則の範囲を先に決める**）。
  (b) 変えず、CONTRADICTION の手検証で除いた件数を報告する（D53 の表の形）。

---


## O32. 相対 URL を分割せず、値全体を `url.host` に入れる — **解決（D57、§9.2）**

- **状況**: `httpx.AsyncClient(base_url=...)` を受け手にした `client.get("/items/" + item_id)` の
  ような**相対 URL** は、`effects._split_url` が「`://` が無い」ので分割せず、値全体を `url.host` に
  入れる。宛先は受け手の `base_url`（多くは OP の設定値）で決まるのに、**パスやクエリのモデル由来が
  宛先のモデル由来に見える。**
- **影響**: run13 の D3 `net_model_host` 14 件のうち code-rag 3 / asquared 3 が誤検出。**`url.host` への
  `GAP_INJECT`（SSRF の座標）にも同じ形で効いているはず**（未測定）。
- **誤りの向き**: 誤警報。
- **要る判断**: (a) 先頭が `/` のリテラルで始まる URL は `url.path` にし、`url.host` は受け手の
  `base_url` から取る（取れなければ `url.host` を置かない = 不明）。**`GAP_INJECT` が減る向きの変更
  なので、消える行を `compare_scans.py` で 1 件ずつ見る。** (b) 変えずに限界に書く。

## O33. 主体 MODEL は「モデルが値を選べる」ではなく「モデルのデータが流れ込む」— **解決（D57、§9.3 / §9.4。「選べる」= MODEL かつ resolved、opaque は不明）**

- **状況**: 原理 3-a（モデルが選べる値は能力として読む）は、slot の主体が MODEL であることで
  「モデルが選べる」を判定している。しかし解析器の MODEL は**流れ込み**の意味で、SQLAlchemy の
  `select(T).where(T.c.in_(値))` のようにモデルの値がバインド変数として入るだけでも、`sql` slot の
  主体は MODEL になる（文の種類はコードが決めていて、モデルは `DELETE` を書けない）。
- **影響**: run13 の D2 `db_model_sql` のうち memory-hub 2 件が誤検出。D3 `net_model_host` の
  dejavu 4 / cohort 4 も、値の形が読めず主体だけ MODEL の形で、選べるのか流れ込むだけなのか分からない。
  **`GAP_INJECT` の `sql` 座標も同じ主体を使っているので、同じ誤警報がありうる**（D17 以来の設計。未測定）。
- **誤りの向き**: 誤警報。
- **要る判断**: (a) 文字列の連結・書式化で作られた値（`Str` の形）だけを「選べる」とみなし、
  形の読めない値（`Unknown`）は不明にする。(b) SQLAlchemy の式（`select` / `insert` / `update` /
  `delete` の戻り値）に型を付け、文の種類を読む（`sql_head` の代わり）。(c) 変えずに限界に書く
  （感度分析の 3-b の行がこの限界の上界になる: 主指標 165 → 153）。

## O34. ゲートの主語一致が「どれか 1 つの根」で、残りの未検証の根まで検査済みにする（誤 clear）— **解決（D58 (a) 厳格版。A2 は 7/8 と報告、argv の分解は O35）**

- **状況**: `gate.subject_ok` は、値検証の候補の主語の根と slot の値の根が**1 つでも**重なれば主語一致と
  する。slot の値に根が複数ある（検証済みの引数と未検証の引数が合流している）と、1 つを検証するゲートが
  slot 全体の要求主体を引き上げ、`GAP_INJECT` を消す。D25 は strong-path の裏付けにだけ「MODEL の根
  **すべて**」を入れ、主語一致には入れていなかった。
- **実例**（O30 の修正が表に出した）: canvas-mcp `update_syllabus` の POST の `body` は根が
  `syllabus_body`（未検証）と `course_identifier`。`course_id` を主語とするゲート候補
  （`make_canvas_request`、config atom `ANONYMIZE_NONE` で等級 OP）が `body` 全体を OP にした。
- **規模**（名前の主語で数えた**上限の目安**。manifest の候補は根ではなく名前しか持たない）: run13 で
  41 位置（canvas-mcp 39 / paper-access 2）、run14 で 66 位置（新しく届いた経路の上で 25 増えた）。
- **誤りの向き**: **誤 clear**（`GAP_INJECT` が消える）。
- **要る判断**: (a) `subject_ok` を「slot の値の MODEL の根すべてが、支配する値検証の主語の根の和集合に
  含まれる」にする（D25 と同じ考え方）。**ゲート（付録）の判定なので、B3a / B3b の fixture・変異試験・
  両側比較への影響を先に見る。** (b) 直さずに限界に書く（上限の目安を併記）。

## O35. argv の要素を「先頭（オプション名）」と「値」に分けて主体を見るか — **未決（D58 の追記で見つけた。凍結後、v3 で事前登録して確かめる）**

- **状況**: フラグ混入（CWE-88）の危険は、モデルが `-` で始まる**要素**を送り込み、コマンドに新しい
  オプションとして読ませることにある。`f"--unified={x}"` のように先頭が定数の `--opt=` の要素では、
  `x` はオプションの**値**にしかならず、新しいオプションは作れない。解析器は argv 要素を 1 つの slot と
  して扱い、要素のどこかに未検証の MODEL の根があれば `GAP_INJECT` を出す。
- **実例**: A2 修正側 `repo.git.diff(f"--unified={context_lines}", target)`。厳格版 O34（D58）で
  `context_lines` が未検証の根として残り、A2 の両側比較が落ちる（8/8 → 7/8）。
- **誤りの向き**: 誤警報の可能性（オプションの値の位置にある未検証の根を、フラグ混入の座標に数える）。
  ただし値の位置でも、オプションによっては値そのものが危険（`--output=`、`--upload-pack=` 等）なので、
  「値の位置なら安全」とは言えない。
- **なぜ今は入れないか**: A2 が落ちたのを見て思いついた分解であり、A2 で作って A2 で確かめても証拠に
  ならない（D58 の追記、CLAUDE.md 規則 5）。
- **要る判断**: (a) 凍結後、規則（要素の分解の仕方と、値の位置を別の座標にするか / 数えないか）を
...
- **状況**: (1) `_ev_UnaryOp` は被演算子の値をそのまま返す（`not x` が `x`、`-1` が `1`）。
  ...
  (3) `env_join` は片側にしか無い名前をその側の値のまま残す（docstring は Unknown と join すると書く）。
- **影響**: O31（D58）の枝刈りに使うと誤 clear になった（run15 で 23 行）。D58 の改訂で、枝刈りは
  「束縛し直さない仮引数」しか読まないので、**枝刈りには効かない**。値の形（`Atom` の定数・None）の表示と、
  それを読む判定（SQL の文の先頭、HTTP のメソッド名など）には効きうる。**向きと件数は未測定。**
...
## O40. 母集団 v2 の枠の数: 文書の 3,827 と抽出スクリプトの 3,764 の食い違い — **不明（記録のみ）**

- `docs/population_v2.md` と `evidence/population_v2/enumeration.json` は src 側に宣言のある repo を 3,827 とするが、
  v2 / v3 の抽出に実際に使った `scripts/sample_population_v2.py` の母集団は 3,764。差 63 のうち 4 は抽出スクリプトの
  同梱コピーの判定で説明できる（`path_class` だけなら 3,768）。**残り 59 は不明。**
- 影響: v2 の抽出は 3,764 から行われたので、報告する枠の大きさは 3,764 が正しい可能性が高い。v2 と v3 の比較には効かない。
- 要る判断: 無し（学生の最終評価の枠を書くときに、どちらの数を使うかだけ注意する）。

## O41. v3 の確認で見つけ、一般に正しい直し方が無いので直さない誤りと見落とし — **既知の限界（D61）**

- **自分で作った一時ファイルの削除を破壊的（D2 の矛）とみなす**: v3 の抜き取り 60 件中の誤 3（`mkstemp` / atomic write の後始末）。
  MCP 仕様の「additive か destructive か」で読めば破壊的ではないが、採った原理（§6）の読み替えになるので直さない。
- **固定のコマンドに `shlex.quote` した値だけを渡す spawn を「モデルが決めるコマンド」とみなす**: 誤 2。
- **関数が返すモジュール**（`_, m = _load_backend(); m.f()`）を解決しない: 見落とし 1。
- **深さの上限**: 見落とし 2（深さ 5 と 7）。O38。
- **sink 語彙に無いライブラリ**（zvec の `upsert`、arcpy など）: 見落とし 1。O39 と同じく語彙は凍結。
- **`flock` のために `"a"` で開くだけの呼び出しを追記（D4）とみなす**: D4 の誤 1。
- **子プロセスの標準入力への書き込みを FS_WRITE の情報行にする**（helm / kubectl の `communicate(input=None)` で誤 4）。D61 G1 で
  直そうとしたが、インタプリタの一覧に無い argv0 でも標準入力はコード実行・ファイル書き込みに届く（`env python3`・`tee` など）ので、
  情報行は保守的な上界として正しい。取り消した（D61 の改訂）。
- **末尾名だけの解決で、組込み型の値のメソッドを木の中の同名メソッドに結ぶ**（`bytearray.append` で誤 2）。D61 G2 で直そうとしたが、
  値の形は型の証明ではなく誤 clear を作った。取り消した（D61 の改訂）。
- 件数は v3 の抜き取りのもので、母集団の割合の推定ではない（開発用の確認。学生の評価で測る）。
