...
## O30. 末尾名の解決がテストファイルの定義に結びつき、段数ぶん連鎖する — **未決（D53 で見つけた）**
...
## O32. 相対 URL を分割せず、値全体を `url.host` に入れる — **未決（D56 で見つけた）**
...
## O33. 主体 MODEL は「モデルが値を選べる」ではなく「モデルのデータが流れ込む」— **未決（D56 で見つけた）**
