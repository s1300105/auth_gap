...
2. **直したことで別の経路が消えていないか。**
   ...
   **野外の run 同士でも同じことを確かめる**（較正対だけでは足りない）:
   ...
   実際に run 3 で tool_package の 6 件（w-jitz10__spotify_mcp。同名関数の追加で解決を
   失った false-clean）と mcp_server の 1 件（w-giak__mnemo-lite。誤った解決が直った結果で、
   正しい消失）をこれで見つけた。**外れた各件について「消えて正しいか」を原ソースで判断する。**
...
3. **差を処理系と解析器に分けて読む。**
   ...
   **関門の率は点推定だけで読まない**（D18）:
   ```bash
   .venv/bin/python scripts/f0a_ci.py                # → docs/f0a_ci.md
   ```
   抽出単位の木を復元抽出した 95% 区間。区間が閾値を跨ぐ行は「境界」。60 / 30 木の
   標本では mcp_server と tool_package の 3 関門が run 1 でも run 2 でも「境界」になる。
   **自分で確かめること**: `evidence/f0a_run2/units.jsonl` から母集団ごとに木を
   数え、サイトが少数の木に集まっていること（上位数本の木が効果サイトの大半を持つ）を
   見る。区間が広い理由はそこにある。アプリ（8 木）の区間はほぼ退化しているので、
   区間の値そのものを論文に書かない。
