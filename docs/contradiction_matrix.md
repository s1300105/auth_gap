...
### 訂正: 「833 件が黙って落ちている」は誤り

**この文書の初版で「`NET` のメソッドが実行時引数の 832 件と SQL 不明 1 件が黙って
落ちている。規則 4 違反で向きは誤 clear」と書いたが、確かめたら間違っていた。**

宣言ありユニットの `NET` 効果 **1,007 件はすべて manifest に行を持っている。**
内訳:

| 行の状態 | 件数 |
|---|---|
| `verdicts: []` + `notes: ["select_manifest_only(assumed_trig)"]` | 802 |
| `UNKNOWN` を持つ（slot の値が解決できなかった） | 131 |
| `GAP_INJECT` + `UNKNOWN` | 71 |
| `GAP_INJECT` のみ | 3 |

**802 件が `verdicts: []` なのは正しい。** 効果も slot も解決できていて、
`readOnlyHint` の上界に `NET` が入っているので**宣言内**である。
`notes` に理由（`trig` が `assumed` なので SELECT 座標は manifest だけに載る）が書いてある。
**黙ってはいない。**

**本当の問題は別にある。**`SLOTS["NET"]` は
`("url.scheme","url.host","url.path","url.query","body","headers")` で、
**HTTP メソッドが slot に無い。** つまり読み / 書きの区別は
「値が解決できない」のではなく「**そもそもモデルに無い次元**」である。
`requests.post` のように site 名でメソッドが判る場合と、
`httpx.AsyncClient.request(method, …)` のように実引数の場合がある。

**したがってこれは規則 4 の違反ではなく、O23 #2（決めが要るマス）の前提となる
モデルの不足である。** cell #2 を採らないなら不足のままでよい。採るなら
`SLOTS["NET"]`（月 3 凍結）に手を入れることになる。
...
### 表を書く作業そのものが見つけた `_sub_kind` の問題 3 件

**(B1) 改行で始まる `SELECT` が `DB_WRITE` に誤分類される。** — **直した**

```python
head = text.strip().split(" ", 1)[0].upper()   # 旧。authgap/effects.py: _sub_kind
```

`" "` だけで切るので、`"\n  SELECT\n    id, ..."` は `strip()` 後も
`head == "SELECT\n"` になり、読み取り語の一覧に当たらず `DB_WRITE` になる。
母集団 v2 で **2 件**（どちらも宣言なしのユニットなので verdict には出ていない）。
**誤りの向きは誤警報（false-positive）。** `split(None, 1)`（空白全般）にした。
**併せて、空白だけの SQL を `DB_WRITE` ではなく `None`（不明）にした**（規則 4。
母集団 v2 に該当 0 件なので観測は変わらない）。

**(B2) `DB_WRITE` は「データを変更する」という意味ではない。** — **直さない。名前の問題**

`_sub_kind` の `DB_WRITE` の定義は「**読み取り語で始まらない**」である。したがって
`PRAGMA`（接続設定）も `BEGIN` / `COMMIT` / `ROLLBACK`（トランザクション制御）も
ここに入る（母集団 v2 で `PRAGMA` 101 / `COMMIT` 2 / `BEGIN` 1 / `ROLLBACK` 1 = **105 件**）。

**これは元の用途（仕様書 §297 の `Leak` 等級づけ。ゲートが読み経路を覆うかの判定）では
正しい。** 問題は、この表の D1 / D2 が要求するのは「**データを変更するか**」という
別の述語である点にある。

**したがって `SUB_KINDS` の語彙は変えない**（月 3 の sink 語彙凍結の対象）。
**表を実装するときに `DB_WRITE` をそのまま「変更」として使ってはならない。**
使うと `PRAGMA` 101 件がそのまま誤警報になる。`_sub_kind` に注記を入れた。

**(B3) `WITH` を無条件に読み取りとするのは潜在的な誤 clear。** — **直さない。記録する**

PostgreSQL などは `WITH x AS (...) INSERT INTO ...` を書ける。読み取り語の一覧に
`WITH` があるので、これは `DB_READ` になる。**向きは誤 clear。**

**母集団 v2 に `WITH` で始まる SQL は 0 件**なので、観測された誤りではない。
条件つきにすると（本文に `INSERT` の語があれば書き込みとするなど）、文字列リテラルや
列名に含まれる語で誤警報を作りうる。**憶測で直さず、既知の限界として記録する**
（`docs/open_questions.md` O24）。
...
2. **(B1) を直す。** — **済（この文書と同時にコミット）。**`_sub_kind` の head の取り方だけ。
   **(B2) は直さない**（`DB_WRITE` の名前と意味の乖離であって、元の用途では正しい）。
   **D1 × `DB_WRITE` を入れるときに「データの変更」を別に判定する。**
   `DB_WRITE` をそのまま使うと `PRAGMA` 105 件が誤警報になる。
3. **`docs/open_questions.md` O22 を直す。** `Path(...).parent` / `.parents[n]` が
   受け手の Path 形を落とし、`readOnlyHint: true` のツールの `mkdir` を落としていた。
   **確認済みの誤 clear で、最小再現も取れている唯一の項目。**
   表の決めが要るマスに依存しないので、**ここが一番先に効く。**
...
**手順 3 は当初「『不明』を verdict として出す経路を作る（833 件）」だったが、
上の訂正のとおりその 833 件は存在しなかったので差し替えた。**
