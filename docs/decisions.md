...
### ~~**§3 の交差行が合格条件を満たした（重要）**~~ — **この節は誤り。D46 で訂正した**

> **2026-09-23 訂正（D46）。** 下の表と結論は誤りである。`scripts/intersection_rows.py` が
> §3 の「`OPAQUE` 行は数えない」（仕様書 553 行目）を実装していなかったため、
> `UNKNOWN` を併せ持つ行まで数えていた。**除外を適用した正しい値は run4 も run5 も
> 1 行 / 1 プロジェクトで、D43 / D44 は交差行をまったく動かしていない。**
> 見かけの 5 → 15 はすべて除外漏れの産物だった。**向きは主張を過大にする側。**
> 以下は記録として残す（CLAUDE.md 規則 6「両方を記録する」）。
...
> **訂正（D46）**: O26 は判断事項ではなくなった。正しい数え方では合格条件を満たさないので、
> **D36 の結論はそのまま成り立つ。**
...
## D46（2026-09-23）O26 の決着: **`intersection_rows.py` が §3 の除外を落としていた。交差行は run4 も run5 も 1 行 / 1 プロジェクト**

### 何がどちら向きに間違っていたか

**私（D45 の記録）は「§3 の交差行が合格条件を満たした（5 行 / 2 プロジェクト → 15 行 / 5
プロジェクト）」と書いた。これは誤りである。向きは主張を過大にする側。**

仕様書 §3（`AUTHGAP_BRIEF_v3.md` 553 行目）はこう書いている。

> **`Leak`（Def 5-b）だけで説明できる行と `OPAQUE` 行は数えない。D だけで説明できる行も除く。**

`scripts/intersection_rows.py` は `OPAQUE` 行（= `UNKNOWN` を持つ行）を除外していなかった。
docstring とコード中のコメントには「OPAQUE 行は数えない」と書いてありながら、
実装は `(v & SELECT) and (v & INJECT)` しか見ていなかった。**書いた規則を実装していない。**
...
**run4 と run5 で完全に同一の 1 行であり、D43 / D44 は交差行をまったく動かしていない。**
見かけの 5 → 15 は**すべて除外漏れの産物**だった。

### この誤りが作っていた逆さまの指標

除外を落とすと、**解析器が未解決を正直に `UNKNOWN` として報告するほど交差行が増える。**
D44 が主体を直して `GAP_INJECT` が増えた結果、`UNKNOWN` を併せ持つ行が 35 → 66 に増え、
それがそのまま「統一の証拠」として数えられていた。
**§3 が `OPAQUE` 行を除外しているのはまさにこれを防ぐためである。**

### 残る 1 行は本物である

`v2-ariffazil__wealth` / `wealth_fx_rate` / `httpx.AsyncClient.get` / slot `url.query`。

* `trig` = **MODEL / traced**（`wealth_mcp/tools/market.py:110` の `if_chain` dispatch、
  `selector_is_model: True`）。母集団で `traced` の 14 ユニットの 1 つ。
* verdicts = `["GAP_INJECT", "GAP_SELECT"]`。**`UNKNOWN` を持たない。**
* `wealth_fx_rate(base, targets, as_of_date)` の引数が `params` に入り
  `httpx.AsyncClient.get(..., params=params)` に届く。

**§3 の文言では「1〜2 行なら『統一の証拠は逸話的』と書く」に当たる。**

### O26 の決着 — **案 (a)。D36 は降ろしたままにする**

**判断事項ではなくなった。** D36 が降ろした根拠（合格条件を満たさない）は、
正しい数え方でも**依然として成り立つ**。しかも run4 の真の値は 1 行 / 1 プロジェクトで、
D36 が引用した 5 行 / 2 プロジェクトより**さらに条件から遠い**。

加えて格下げ条項（`traced` 率 0.6% < 20%、**撤回しない**と §3 が明記）も発火したままである。

**D36 の結論は変わらない。引用していた数値だけが誤っていた**ので、ここで訂正する。

### 直したもの

* `scripts/intersection_rows.py`: `OPAQUE_VERDICT` と `D_ONLY_VERDICTS` を明示し、
  §3 の除外を適用する。除外件数も出力に載せた（落としたことが見えるように）。
* `docs/intersection_rows.md`: run5 で再生成。
* `docs/decisions.md` D36 / D45 と `docs/preregistration.md` 逸脱 #11 / #12 の
  引用値に訂正を併記する（**元の記述は消さない**）。

### 教訓（本文の限界節に書く）

**「規則を docstring に書いた」ことと「規則を実装した」ことは別である。**
この 2 つが食い違い、しかも食い違いの向きが主張に有利だったため、
**あやうく事前登録した判断を巻き戻すところだった。**
事前登録の指標は、閾値だけでなく**除外規則も**テストで固定する。

---

## D47（2026-09-23）事前登録した規則が実装されているかを点検する。**2 件の不一致**

**きっかけ**: D46 で `scripts/intersection_rows.py` の docstring と実装が食い違い、
しかも向きが主張に有利だったことが分かった。**同じ食い違いが他にもないかを点検した。**
手続きと結果は `docs/rule_implementation_audit.md`。

**網羅的な点検ではない。** `AUTHGAP_BRIEF_v3.md` を
「数えない / 含めない / 除く / に限る / 算入しない」で検索して除外・限定の規則を
**14 件**抽出し、そのうち**実装があり機械的に照合できる 9 件**を突き合わせた。
手作業の項目（κ、ラベリング、較正対の選定）と実装が無い項目（D_dom のパーサ）は対象外。

### 一致していた 7 件

`r_kind` の分子（L320、10 例で確認）/ 未 join 行の扱い（L322）/ `r_dom = 0`（L7-8）/
`self_granted` が weak 理由でなく `downgrade`（L632）/ `dash_rejected` が val の属性に
無いこと（L282）/ D_dom の被覆判定（対象外）/ D46 で直した交差行。

### 不一致 1 — snake_case の注釈が記録されない（**O27**）

仕様 L322 は「snake_case は `D_malformed` として別行で報告する」と定めるが、
**デコレータ経路が `_malformed` を捨て、`DKind.malformed` を設定する箇所が無い**ので
構造的に常に空。母集団 v2 に **574 箇所 / 11 木**あるのに manifest は 0 件。
**仕様の前測は「21 エントリ」だったが桁違いに多い。**

**向きは誤 clear ではない**（snake_case は正しく上界を動かさない）。
**失われているのは測定である。**「読み取り専用のつもりで宣言したが protocol に
届いていない」という具体的な失敗形が、注釈なしのツールと区別できていない。
**「宣言はどれだけ当てにならないか」という本研究の問いに直接効く。**

### 不一致 2 — `db_unresolved` が記録されずに消える（**O28**）

仕様 L1118 は「解決できない `.execute()` は `db_unresolved` として**記録し**、
危険効果に数えない」と定めるが、`effects.py:561` は `return None` で効果ごと捨てる。
**「数えない」は満たすが「記録する」を満たさない。**
`report.py:169` の該当分岐は**到達不能**で、`effect_fp_audit.db_only_non_db_execute` は
**構造的に常に 0**。

**向きは規則 4 違反。**「DB の判定に曖昧さは無かった」と読める出力を出しており、
受け手型が解決できない `.execute()` は黙って消えている。
**その中に本物の DB 書き込みがあっても分からない。**

### 共通する形と対策

3 件（D46 の交差行、O27、O28）はいずれも
...
という形だった。**対策は「除外したもの・落としたものの件数も出力に載せる」こと。**
D46 で `intersection_rows.py` に `n_excluded_opaque` を足したのがその形で、
O27 / O28 を直すときも同じにする。
...
## D53（2026-09-24）深さの上限の感度分析（O29）。**解析器は変えていない**（学生の判断材料）

`scripts/scan_v2.py --max-depth <d>` で `Options.max_depth` だけを変え、母集団 v2（87 木）を
深さ 3 / 4 / 5 で取った。`MAX_DEPTH = 3`（事前登録の上限）は変えていない。
集計は `scripts/depth_sensitivity.py`、表は `docs/depth_sensitivity.md`（再生成できる）。

### 件数（`evidence/scan_v2_depth{3,4,5}_sens`）

| 項目 | 深さ 3 | 深さ 4 | 深さ 5 |
|---|---|---|---|
| 効果（生の件数） | 7,503 | 11,500 | 17,694 |
| 効果の位置（一意） | 3,185 | 4,050 | 4,228 |
| DB 効果 | 1,741 | 2,490 | 2,989 |
| FS_WRITE | 326 | 2,340 | 3,899 |
| **DB 未解決率** | **24.6%** | **17.6%** | **14.2%** |
| `GAP_INJECT`（一意な位置） | 994 | 1,108 | 1,145 |
| **CONTRADICTION**（ユニット × site × kind） | **85** | **146** | **193** |
| 深さの上限に当たったユニット | 1,642 | 805 | 696 |

**消えたもの（ユニット・効果の位置・経路・`GAP_INJECT`・CONTRADICTION）は、
3 → 4、3 → 5、4 → 5 のどれでも 0。** 深さ 3 の run は run10 と全項目一致（決定論の確認）。
解析の失敗・再帰の打ち切りは 0。構文解析の失敗 86 件・`ast_node_cap` 135 件・
`meta-skill-evloving` の打ち切りは深さと無関係で 3 深さとも同じ。

### 費用（`evidence/scan_v2_depth{3,4,5}_time`、同条件）

感度分析の run は同時実行と単独実行が混在して時間を比べられないので、
`meta-skill-evloving`（前処理だけで深さと無関係）を除いた 86 木を、深さ 3 → 4 → 5 の順に
**1 プロセスずつ単独で**取り直した。件数は感度分析の run と全木一致した。

| | 深さ 3 | 深さ 4 | 深さ 5 |
|---|---|---|---|
| 86 木の合計 | 396 秒 | 424 秒（+7%） | 490 秒（+24%） |
| 深さ 5 / 深さ 3 の比（1 秒以上の 39 木の中央値） | | | 1.03 倍（最大 3.7 倍） |
| tree budget の打ち切り（86 木） | 0 | 0 | 0 |
| 1 ユニット 10 秒超（`WALL_CLOCK_CAP`） | 1 | 1 | 1（同じユニット） |

伸びたのは `canvas-mcp`（25 → 48 → 80 秒）と `mcp-linkedin`（9 → 16 → 28 秒）。
**費用は障害にならない。** 同時実行は `meta-skill-evloving` の前処理（1 本 7〜10 GB）で
OOM になるので、深さによらず 1 本ずつ走らせる（D52）。

### 増えた CONTRADICTION 108 件（3 → 5）の全件検証

原典を読み、`witness_chain` の各段の分岐条件を確かめた（3 → 4 の 61 件と 4 → 5 の 47 件）。

| 判定 | 3 → 4 | 4 → 5 | 計 | 中身 |
|---|---|---|---|---|
| **本物** | 35 | 28 | **63** | `mcp-linkedin`: `readOnlyHint: True` のツールが、エラー時に診断ファイルを書く（`secure_write_text`）・導入済みの印を消す（`invalidate_browser_setup`）・ブラウザのプロファイルを退避する（`shutil.move`）。`ros-mcp` の `subscribe_*` 2: `readOnlyHint=True` で、モデルが渡す `expects_image` が真なら `./camera/received_image.jpeg` を書く |
| **遠隔の応答次第** | 13 | 15 | **28** | `ros-mcp`: 示した連鎖（`request` → `_handle_image_hint`）は `expects_image=None` なので実行されない。**別の経路**（自動判定 → `parse_image`）で、rosbridge が画像の `publish` を返したときだけ書く |
| **規則どおりだが意味は非破壊** | 8 | 4 | **12** | `cohort` 8: JSON ストアの読んで足して書き戻す（`write_text`、意図は追記）。`yt-scribe` 1: 新しい一時ディレクトリの新規ファイル。`ly-ads` 2: 自分で作ったレポートの失敗時の後始末。`innovatehub` 1: `git describe --tags`（D32 で SPAWN を破壊的に扱う。O23 のマス） |
| **誤検出** | 5 | 0 | **5** | `yt-scribe` 3: MCP 経路は `out_path` を渡さず `write_text(None, …)` が先頭で戻る。`tg-note` 1: `sync_models(force=False)` で `if force and …: shutil.rmtree` に入らない。`cohort` 1: Playwright の `page.route(...)` を木内の `LocalRouter.route` に末尾名で結んだ |

* **本物 63 件のうち 9 件はテストファイルの中で定義されたツール**（`mcp-linkedin/tests/test_daemon_auth.py`）。
  テストコードを母集団に入れるかは深さと別の問題（深さ 3 でもこれらのユニットはある）。
* 誤検出 5 件のうち 4 件は **`None` / `False` の定数引数による分岐**を見ない（経路非感応）。
  深さ 3 では上限の手前で止まって見えていなかった。
* 深さ 3 の CONTRADICTION 85 件のうち 78 件がすでに `opaque(unresolved)` の効果の上にある
  （D17 の「末尾名で降りて確度を落とす」）。深さを上げると同じ形の連鎖が伸びる。

### 増えた効果の問題: **末尾名の誤解決が段数ぶん連鎖する**

深さ 3 → 4 で増えた経路 1,651 件のうち **430 件（26%）が 1 本の連鎖**だった
（`xorbitsai__xagent`）: `logger.error(...)` → テストファイル内の関数に入れ子で定義された
`_Recorder.error`（`tests/core/test_execution_scope.py:67`）→ 末尾名の `append` →
`append_task_execution_event_no_commit` → … **テストファイルの中にある効果が 0 → 348 → 389。**
各段は `opaque(unresolved)` を持つので D17 の規則どおりだが、**1 段深くするたびに
末尾名の結びつけがもう 1 回重なる。** これは CONTRADICTION ではなく `UNKNOWN` の行に出るので
上の 108 件には入らない。

一方、深さ 4 → 5 で増えた経路 1,662 件の半分（825 件）は `canvas-mcp` の 84 ツールが
`get_course_id` → `refresh_course_cache` → `make_canvas_request` で Canvas API に届く
**本物の通信**（同じ位置への別経路）だった。

### まとめ（判断は学生。O29）

* **得るもの**: DB 未解決率 24.6% → 17.6% → 14.2%。CONTRADICTION は 85 → 146 → 193 で、
  増えた 108 件のうち**本物 63・遠隔次第 28・意味は非破壊 12・誤検出 5**。消えたものは 0。
* **費用**: 時間 +7%（4）/ +24%（5）。打ち切りは増えない。
* **失うもの**: 経路非感応による誤検出（定数 `None` / `False` の分岐）と、末尾名の誤解決の
...
## D58（2026-09-24）凍結の前に O31 / O34 を直す — **規則（実装より先にコミット）**

学生の決定: 解析器を凍結する前に O31 と O34 を直す。規則をここに先に書き、テスト、実装の順に進める。

### O31: 定数の引数で決まる分岐を刈る（誤警報の修正）

1. **省略された引数の既定値**: 呼び出し先に降りるとき、省略された仮引数の既定値が**リテラルの定数**
   （`None` / 真偽 / 数 / 文字列）なら、その定数（主体 OP、確度 resolved）を入れる。今までは注釈から
   作った「値の分からない」仮引数にしていた。注釈による受け手の型付け（D50）はこの後に今までどおり
   かかるので、`db: Session = None` の型は失われない。**入口のユニット自身の仮引数（MODEL）は変えない。**
2. **`if` の条件の静的評価**: 条件が、確度 resolved の定数に束縛された**名前**とリテラルだけから決まる
   とき（`not x` / `x and y` / `x or y` / `x is None` / `x is not None` / `x == c` / `x != c`）、取られる側の
   枝だけを実行する。属性（`self.x`）・呼び出し・添字を含む条件は評価しない（両方の枝を実行する。今までどおり）。
3. **終わる枝**: `return` / `raise` の後の同じ本体の文は実行しない。`if` は、取られる側（条件が決まらない
   ときは両方）の枝が必ず終わるときだけ「終わる」とする。`try` / `with` / ループは終わるとみなさない
   （保守側）。終わる枝で捨てる env からも、戻り値（`<return>` / `<yield>`）は合流する。
4. 呼び出し先は呼び出しごとに実行し直している（結果を使い回していない）ので、呼び出し元ごとの刈り込みは
   ほかの呼び出し元に漏れない。

**誤 clear の危険**: 条件の評価を誤ると、実行される枝を捨てる。定数は確度 resolved のものだけを使い、
名前以外は評価しない。**解決を上げる変更なので、敵対的なテスト（刈ってはいけない形）を先に書く。**

### O34: ゲートの主語一致は、値の MODEL の根**すべて**に要求する（誤 clear の修正）

制御位置の値に根が 2 つ以上あるとき、`score_gates` を**根ごとに**呼び、要求主体はその**最も弱いもの**
（`req_meet`。攻撃者は検証されていない根を使う）にする。表示するゲートの結果と等級も最も弱い根のものを
使う。根が 1 つ以下のときは今までと同じ。D25 が strong-path の裏付けにだけ入れた「MODEL の根すべて」を、
主語一致にも入れる。**ゲート（付録）の判定なので、B3a / B3b・変異試験・両側比較への影響を必ず見る。**

### D58 の追記（2026-09-24）: 厳格版の O34 が A2 の較正を落とす — **規則は変えず、A2 は 7/8 と報告する**

**観察**（実装を試し、コミットせずに戻した段階。O31 は `860aa93` でコミット済み）: 上の規則どおりに
O34 を実装すると、`two_sided.py` の両側比較が 8/8 から 7/8 になる。落ちるのは **A2**
（CVE-2025-68144、フラグ混入）の修正側で、`repo.git.diff(f"--unified={context_lines}", target)` の
`argv` に `GAP_INJECT` が残る。修正で足された `target.startswith("-")` の検証は `target` の根にしか
効かず、`context_lines`（低レベル MCP の `arguments.get("context_lines", ...)`。`GitDiff` の `int` 型は
`list_tools` のスキーマに使われるだけで、ハンドラの中では検証されない）は未検証の MODEL の根として残る。

**検討した案と採らなかった理由**:
- (案 1、**取り下げ**) argv の要素が定数の `--opt=` で始まるときは、その中の根を数えない例外を足す。
  A2 が落ちたのを見てから、A2 の形に合わせて足す規則であり、CLAUDE.md 規則 5（凍結した項目を
  結果を見て変えない）に反する。逸脱として開示しても、較正に合わせた調整であることは変わらない。
  学生の指摘（「あまり綺麗ではない」）で取り下げた。

**決定（学生の了承、2026-09-24）**:
1. **規則は上のまま（厳格版）で実装する。** A2 の両側比較は **7/8** と報告する。
   **これまでの A2 の 8/8 は、O34 の誤 clear（`target` の検証が `context_lines` の位置にも付いていた）に
   一部支えられていた。** これ自体を結果として書く。`fixtures/*/expected.json` と
   `docs/expected_tuples.json` は凍結済みなので変えない。A2 を期待する 2 件のテスト
   （`test_two_sided_calibration` の A2、`test_preregistered_direction_passes[A2]`）は
   **`xfail(strict=True)` にし、理由に本項を引く**（通るようになれば落ちて気づく。期待値そのものは消さない）。
2. **「argv の要素は先頭（どのオプションか）と値（`=` の後ろ）に分けて主体を見るべきでは」は
   `docs/open_questions.md` O35 に未決として置く。** URL を host / path に、SQL を先頭の文と残りに
   分けたのと同じ種類の分解で、筋のある一般論だが、**A2 を見て思いついた**ので今は入れない。
3. **採るなら、規則を先にコミットし、A2 を含まないデータ（母集団 v3）で確かめる。** A2 で作って
   A2 で確かめても証拠にならない。凍結後の変更なので逸脱として記録する。

### D58 の改訂（2026-09-24）: O31 の「静的な名前」を狭める — **run15 の敵対的レビューで誤 clear が見つかった**

**見つかったこと**（run14 → run15 で消えた効果を、木ごとに 3 つの独立したレビューで原ソースと突き合わせた。
canvas-mcp の 2,076 件は別に確かめた）:

| 群 | 消えた行 | 正しい | **誤 clear** |
|---|---|---|---|
| canvas-mcp | 2,076 | 2,076 | 0 |
| memory-hub | 110 | 88 | **22** |
| teamplay-talk ほか 7 木 | 127 | 126 | **1**（auto_grocer） |
| claude-replay / cohort / release-kit / xagent | 69 | 69 | 0 |

**誤 clear の原因**（どれも O31 の規則 2「確度 resolved の定数に束縛された名前」が、名前の値の出どころを
問わなかったことによる。値の側の欠陥は O31 より前からあるが、O31 がその値で枝を捨てたので誤 clear になった）:

- (A) **呼び出し側の `**kwargs` / `*args`**: `_ev_Call` は `**` の実引数を捨て、`*` の要素を 1 つの位置引数にする。
  O31 の規則 1 はそこへ省いた仮引数の既定値を入れるので、実際には渡される `focus` / `include_provenance` /
  `content` などが `None` / `False` の定数になった（memory-hub の `_dispatch_*` が `**_forward(opts)` で渡す）。
- (A') **間接の降下**（`asyncio.to_thread` / `Executor.submit` / `Process(target=)`）: `args` の要素だけを渡し、
  キーワード引数を渡さない。残りが既定値の定数になる。
- (B) **`not` の代入**: `_ev_UnaryOp` は被演算子の値をそのまま返すので、`cache_optimized = not raw_results` が
  `raw_results` と同じ真偽になる（逆）。`-x` / `~x` も同じ。
- (C) **None との合流**: `ir._shape_join` は `Atom` の `none` を比べないので、`join(Atom(none), Atom())` が
  `Atom(none)` になる（順序で結果が変わる）。MODEL の値（`extract_mode`）が `if` の後で定数 None になった。
- (D) **片側だけの束縛**: `env_join` は片側にしか無い名前をその側の値のまま残す（docstring は「Unknown と
  join する」と書くが実装が違う）。`try: from m import f` / `except: f = None` の後の `f` が定数 None になり、
  `if f and f(url):` の枝が捨てられた（auto_grocer の `requests.get`）。

**改訂した規則**（O31 の規則 1〜3 を次で置き換える。規則 3「終わる枝」と規則 4 は変えない）:

1. **静的な名前は、いま実行している呼び出し先の仮引数に限る。** 局所変数（本体で代入された名前）・大域の名前・
   import した名前は、値が定数に見えても静的にしない。(B)(C)(D) はすべて局所変数の値なので、この規則で当たらない。
2. 仮引数が静的なのは、次のすべてを満たすときだけ:
   - (a) **呼び出しが直接の呼び出し**（`ast.Call` で、実引数に `*` も `**` も無い）。間接の降下（A'）は静的な
     仮引数を持たない。
   - (b) 値の出どころが、**省いた仮引数のリテラルの既定値**、または**実引数の式がリテラル**
     （`None` / 真偽 / 数 / 文字列の `ast.Constant`。`-1` のような単項演算は含めない）、または**実引数の式が
     呼び出し元で静的な名前**。
   - (c) **呼び出し先の本体のどこでもその名前を束縛し直さない**（代入・拡張代入・注釈つき代入・`for` /
     `with` / 内包の対象・`except ... as`・import の別名・`global` / `nonlocal`・`del`・`:=`・`match` の捕捉。
     入れ子の関数の中も含めて保守的に見る）。
3. **省いた仮引数にリテラルの既定値を入れる（旧規則 1）のは、2(a) を満たす呼び出しだけ。** `*` / `**` の
   ある呼び出しと間接の降下では、O31 より前と同じく注釈から作った「値の分からない」仮引数にする
   （既定値を入れると、実際には渡されている値の主体が OP の定数に置き換わり、値の側でも誤 clear になる）。
4. `if` の条件の評価（旧規則 2）は、静的な名前とリテラルだけを読む。形（`not` / `and` / `or` / `is None` /
   `==` / `!=`）は変えない。

**(B)(C)(D) の値の欠陥そのものは直さない**（O31 より前からあり、値の形の表示に効くが、改訂後は枝刈りに
使われない）。`docs/open_questions.md` O36 に置く。

**誤りの向き**: 改訂前は**誤 clear**（到達する効果を捨てる）。改訂は刈る範囲を狭めるだけなので、run15 で
正しく刈れていた形のうち、局所変数で決まるもの（例: cohort の `status == "complete"` は仮引数なので残る、
lara-unb の `expects_image=None` は仮引数なので残る）以外は残る見込み。**run16 で、上の誤 clear 23 行が
戻ること、正しかった刈り込みがどれだけ残るかを両方数える。**

### D58 の結果（2026-09-24、`scan_v2_run16`。解析器 `04cd69b` = O31 の改訂 + 厳格版 O34）

**規則は実装の前にコミットした**（規則 `6da93f6`・A2 の扱い `0c36cbb`・改訂 `82e3258`。テストは各実装の前）。
run15（改訂前の O31）は敵対的レビューで誤 clear が見つかったので**採らない**。記録として残す。

| 項目 | run14（変更前） | run15（O31 改訂前、不採用） | **run16（採用）** |
|---|---|---|---|
| 効果 | 10,193 | 7,847 | **7,915** |
| 危険なユニット | 1,346 | 1,350 | **1,350** |
| CONTRADICTION（ユニット × site × kind） | 191 | 189 | **189** |
| **D1 + D2 の矛**（主指標） | 154 | 151 | **151** |
| D3 矛 / D4 矛（探索的） | 38 / 0 | 39 / 0 | **39 / 0** |
| GAP_INJECT の行 | 2,810 | 1,695 | **1,705** |
| 両側比較（腕 C、事前登録照合 / 座標一致 / any-change） | 8/8 / 8/8 / 8/8 | 7/8 / 7/8 / 8/8 | **7/8 / 7/8 / 8/8** |

**消えた効果の確かめ方**（CLAUDE.md の「解決率を上げる変更は敵対的レビューを通す」）:

- run14 → run15 で消えた効果 2,424 件を全件見た。canvas-mcp の 2,076 件は `make_canvas_request` の
  `files=None` / `use_form_data=False`（既定値）で到達しない POST の枝（本記録者が原ソースで確認）。
  残り 348 件は木ごとに独立した 3 つのレビューで原ソースと突き合わせ、**誤 clear 23 行**（memory-hub 22・
  auto_grocer 1）を見つけた → 上の「D58 の改訂」。
- run16 では、run15 で消えたものの**部分集合**だけが消えている（run15 で消えず run16 で消えた効果 = 0）。
  誤 clear だった 23 行は**経路の単位ですべて戻った**。戻ったのは 78 件（memory-hub 75、auto_grocer 1、
  quantcontext 2）で、残り 55 件は正しかった刈り込みを保守側に戻したもの（`**` 経由の定数など）。
  **したがって run16 で消えた 2,346 件はすべて、レビューで正しいと確かめた集合に入る。**
- 増えた効果 68 件（codegrok 4・apk-preview 57・nan-fe 7）は、既定値 `None` が `x or Cls()` の受け手の型を
  決めて解決が進んだもので、どれも到達する sink（レビューで確認）。run15 で増えた napjon の 10 件は
  片側だけの束縛による誤った前提で出ていたので、run16 では消えた（O36）。
- 消えた CONTRADICTION 3 件: tg-note `rmtree`（`force=False` で到達しない。正しい）、yt-scribe の
  `mkdir` / `write_text` 2 件（`write_text(None, ...)` の経路は到達しないので消えたのは正しいが、**本当の書き込みは
  深さ 5 の別経路にあり、深さの上限による誤 clear**。O38）。増えた 1 件: nan-fe `get_lark_auth_status` の POST
  （D3 `openWorldHint: false` と外部ホスト。正しい）。
- GAP_INJECT の減少（2,810 → 1,705）はほぼすべて消えた効果の行（canvas-mcp 1,076 ほか）。厳格版 O34 は
  生き残った行に GAP_INJECT を**足すだけ**（canvas-mcp 16・paper-access 4 の 20 行。消した行は 0）。生き残った行で
  GAP_INJECT が外れた 24 行（mcparmory google-sheets）は O31 による `data=` の枝の刈り込みで正しいが、
  モデル由来の本体は `json=` 側にあり sink 表が見ていない（O37）。

**残した限界**: O35（argv の分解）、O36（値の欠陥 3 つ。枝刈りには使わない）、O37（`request` の `body`）、
O38（深さ 5 の書き込み・`finally` の `unlink`）。

## D59（2026-09-24）O36 / O37 を直して凍結する — **規則（実装より先にコミット）**

学生の決定: O36 と O37 を直す。O38 の深さの限界は直さず、論文で深さを変えたときの変化を評価して書く
（D53 / D54 の感度分析の形。凍結後の解析器で深さ 3 / 4 / 5 を取り直す）。

### 凍結の条件（**先に決めておく**）

母集団 v2 は誤りを探すのに何度も使ったので**開発用のデータ**であり、主張の評価には使わない。評価は凍結後の
母集団 v3 で行う。**D59 の実装・取り直し・消えた行の確認を終えた時点で解析器を凍結する。凍結後に見つかった
欠陥は直さず、既知の限界として記録する**（直すなら `docs/preregistration.md` に逸脱として書き、v3 の結果は
凍結時の解析器のものを主とする）。

### O36: val エンジンの値の欠陥（正解は Python の意味で決まる）

1. **単項演算**（`_ev_UnaryOp`）: 結果は被演算子の値ではなく演算の結果にする。
   - `not x`: 被演算子の形が `Atom(none)` なら定数 `True`、定数（真偽・数・文字列）なら `not c`、それ以外は `Atom()`。
   - `-x` / `+x` / `~x`: 被演算子が数の定数なら計算した定数、それ以外は `Atom()`。
   - 主体・確度・root は被演算子のものを保つ。**属性（`attrs`）は落とす**（検証の印を別の値に持ち越さない）。
2. **形の合流の可換性**（`ir._shape_join`）: `Atom` どうしを同じとみなすのは、`none`・`formal`・定数の**型と値**が
   すべて等しいときだけ（`True` と `1` は別）。今は `none` を比べず、`join(Atom(none), Atom())` が順序で変わる。
3. **片側だけの束縛**（`env_join`）: **片側にしか無い名前をその側の値のまま残すのは正しい**（他方の経路でその
   名前を読めば NameError / UnboundLocalError で止まるので、読める経路の値はその側の値だけ）。docstring
   （「Unknown と join する」）が誤っていたので直す。**本当の欠陥は、実行時には束縛されるのにエンジンが
   束縛を記録しない文**:
   - **関数本体の `import` / `from ... import`**: 実行時に名前を束縛し直すのに、env に何も書かない。
     `try: from m import f` / `except: f = None` の後の `f` が定数 None になる（auto_grocer / napjon）。
     → `import` の文で、束縛される名前（`asname`、無ければ先頭の成分。`*` は除く）を、**その名前を env の外で
     読んだときの値**（`_module_value` の値、無ければ `opaque(unresolved)` の `Atom(formal=名前)`）に束縛し直す。
     `import` の無い経路の結果は今までと同じ（名前を読むときの値と opaque の記録を変えない）。
4. **`except` 節の入口の env**（同じレビューで見つかった同じ種類の欠陥。O36 に含める）: 今は try の**前**の env から
   始めるので、try 本体の代入を見ない（`proc = None; try: proc = Popen(...)` / `except: proc.kill()` の `proc` が
   None のまま、`kill` の効果が消える。apk-preview）。→ **try の前と、try 本体の各文（最上位）の後の env の合流**
   から始める（例外はどの文の途中でも起きうる。入れ子の文の途中の状態は文単位に丸める）。

**誤りの向き**: 1・2・3・4 とも値を誤って定数にする（1 は逆の真偽、2・3 は None、4 は古い値）ので、
**誤 clear**（受け手の型が落ちて効果が消える / 定数に見えて矛盾の分類が変わる）の向き。直すと効果が**増える**
向き（4）と、定数が `Atom()` に変わって矛が不になる向き（2・3）がありうる。**`compare_scans.py` で両方向を
1 件ずつ見る。**

### O37: HTTP の `body` を仕様書どおり `data|json` にする

仕様書 229 行目: 「httpx / requests の `Client` / `Session` の `.get` / `.post` / `.request` →
`NET { url ← arg0, body ← data|json }`」。**実装は行ごとに片方しか引いていなかった**（`requests.post` /
`put` / `patch` / `request` とセッションの `request` は `data=` だけ、`httpx.post` とセッションの `post` は
`json=` だけ、`httpx.request` は `body` 無し）。**仕様からのずれなので、仕様に合わせる**:

- `body` を持つ HTTP の行（直接・proxy）は、`body ← data= ⊔ json=`（両方あれば合流、片方ならその値）にする。
  セッションの `get` と `httpx.request` にも仕様どおり `body` を足す。
- **仕様書に無いものは足さない**（sink 語彙は月 3 凍結、仕様書 841 行目）: `content=` / `files=`、セッションと
  httpx のモジュール関数の `put` / `patch` / `delete` / `head` / `options`。**凍結前に件数を測って既知の限界として
  書く**（O39）。
...
### D59 の結果（2026-09-24、`scan_v2_run18`。解析器 `90ef6dd`）と凍結

**規則は実装の前にコミットした**（D59 `693ee79`、追記 `5ab87cd`。テストは各実装の前: `ab7d177`・`f77aaa6`）。run17（追記の前）は中間の記録。

| 項目 | run16（D58 後） | run17（O36 / O37） | **run18（+ url=、凍結）** |
|---|---|---|---|
| 効果 | 7,915 | 8,229 | **8,254** |
| 危険なユニット | 1,350 | 1,508 | **1,518** |
| CONTRADICTION（ユニット × site × kind） | 189 | 189 | **189** |
| **D1 + D2 の矛**（主指標） | 151 | — | **151** |
| D1 矛 / 不 · D2 矛 / 不 | 107 / 95 · 44 / 107 | — | **107 / 95 · 44 / 108** |
| D3 矛 / 不 · D4 矛 / 不（探索的） | 39 / 99 · 0 / 71 | — | **39 / 99 · 0 / 72** |
| GAP_INJECT の行 | 1,705 | 2,020 | **2,290** |
| NET 効果のうち `url.host` の無いもの | — | 1,693 | **6**（`httpx.AsyncClient.get`） |
| 両側比較（事前登録照合 / 座標一致 / any-change） | 7/8 / 7/8 / 8/8 | 同 | **7/8 / 7/8 / 8/8** |

**消えた行の確かめ**: run16 → run18 で**消えた効果 0、消えた CONTRADICTION 0、消えたユニット 0**。増えた効果 339 件は
すべて xagent の `requests.request` / `httpx.AsyncClient.request` で、全部をキーワードで渡す呼び出しが `json=` と
`url=` を引けるようになって効果になったもの（原ソースで確認: `slack.py:241` ほか）。「root があるのに主体が OP」の
slot が 39 → 43（+4）: xagent `onedrive_upload_file` の `url.path` で、深さの上限で降りなかった値
（`opaque(depth)`、主体 OP・root つきで返す既存の設計）が、宛先を引けるようになって表に出たもの。新しい欠陥ではない。

**残した限界**（凍結後は直さない）: O35（argv の分解）、O38（深さ 5 の書き込み。論文で深さ 3 / 4 / 5 を評価して書く、
`finally` の `unlink` は原因**不明**のまま）、O39（仕様書の語彙に無い HTTP のメソッドと `content=` / `files=`。
構文で数えた上限: 呼び出し箇所 150 と 143、`docs/o39_count.json`）。`Seq` などの入れ子の値の等値比較は dataclass の `==`
（`True == 1`）のままで、入れ子の中の Atom の合流は O36-2 と同じ非可換が残る（件数未測定）。
...
## D60（2026-09-24）論文の評価は学生が別の新しいデータで行う — `analyzer-freeze-1` は中間の区切り、v3 は開発用の確認

**学生の決定**: 論文用の比較表はこちら（本記録者）では作らない。システムの完成後に、学生が別の新しいデータで
手動で評価する。本記録者は新しいデータ（v3）で解析器を確かめる。

**D59 の凍結の条件を変える**（理由を先に書く）: D59 は「凍結後に見つかった欠陥は直さず限界として記録する」とした。
これは v3 が評価データになる前提での規則で、評価データを見た後に解析器を直すと評価が甘くなるのを防ぐものだった。
**評価が学生の別データで行われるなら、v3 は開発用であり、v3 で見つけた欠陥を直しても評価は汚れない。**
守るべきことは「評価データを本記録者が一度も見ない・開発に使わない」ことに移る。したがって:

1. `analyzer-freeze-1`（`1bcace3`）は**中間の区切り**とする。v3 で見つけた欠陥は今までと同じ手順
   （規則 → 反例テスト → 実装 → 取り直しと突き合わせ）で直す。
2. **学生の最終評価の前に解析器をもう一度凍結する**（最終凍結。`scripts/freeze_analyzer.py` とタグ）。
3. 学生の評価データには v2 と v3 の repo を含めない（`docs/corpus_sample_v2.json`、`docs/corpus_sample_v3.json`）。
4. v3 の枠・抽出・確かめ方は `docs/population_v3.md` に、抽出の前にコミットした。

## D61（2026-09-24）v3 の所見の扱い: **検証データに合わせて直さない。一般に正しい直し方だけを入れる** — 規則（実装より先にコミット）

**学生の方針**: 検証データを見て変更するのはきりがなく、研究として良くない。誤りの修正が一般的に役立つものなら改善する。

**基準**（v3 の所見ごとに、次の両方を満たすものだけを直す）:
1. **正解が v3 と無関係に決まる**: Python の言語の意味・ライブラリの仕様・既存のカタログから導ける。v3 の件数や個々の
   事例に合わせた閾値・例外ではない。
2. **一般的に効く**: 特定の木の書き方への対応ではなく、同じ形がどの木にも現れうる。

**直すもの**（それぞれ規則 → 反例テスト → 実装 → v2 と v3 の取り直しの順。v3 の取り直しは**回帰の確認だけ**に使い、
v3 の精度が上がったことを結果として報告しない — v3 は開発用で、見た後に直したので評価には使えない）:
...
- **G4 注釈で型を付ける受け手の型を、sink 表の proxy の受け手の型から導く**: D50 / D51 は `ANNOTATION_TYPED_RECEIVERS`
  ...
  クライアントなど）。**一覧は手で足さず、sink 表から機械的に作る。**
- **G5 解決できなかった木の中の呼び出しを数えて出す**（判定は変えない）: 規則 4 の趣旨（黙って安全側に倒さない）。
  ユニットごとに、名前が木の中の定義に当たるのに降りられなかった呼び出しの数を manifest に出す。「矛盾なし」の確からしさを
  読み手が判断するための列であり、verdict には使わない。

**直さないもの**（既知の限界として `docs/open_questions.md` O41 に書く）:

- **自分で作った一時ファイルの削除を破壊的とみなす**（誤 3）: 「破壊的」の定義の読み方の問題で、採った原理（§6）を
  v3 を見た後に読み替えることになる。
- **`shlex.quote` した値だけを渡す固定のコマンド**（誤 2）: 「モデルがコマンドを決める」の読み方と、引用されたコマンドの
  意味の判断が要る。
- **関数が返すモジュール**（見落とし 1）: 珍しい書き方への対応になる。
- **深さの上限**（見落とし 2）: O38 と同じ。論文で深さを変えて評価する。
- **語彙に無いライブラリ**（zvec / arcpy など、見落とし 1）: sink 語彙は月 3 凍結（O39 と同じ扱い）。
- **`flock` のための `"a"`**（D4 の誤 1）。

**この後は v3 を見て直さない。** G1〜G5 を終えたら最終凍結し、学生の評価に渡す。

### D61 の改訂（2026-09-24）: 敵対的レビューで G1・G2 は一般に正しくないと分かったので取り消す。G3・G4 は健全性の条件を足す — **規則（実装より先にコミット）**

`672f0f0` の敵対的レビュー（入力を作って 1100319 と 672f0f0 を走らせ比べた。再現の木は scratchpad の `advd61/trees`）で:

- **G1 は前提が誤り**: 「子プロセスの標準入力はファイルではない」は一般には成り立たない。インタプリタの一覧に無い argv0 でも、
  標準入力のデータはコード実行やファイル書き込みに届く（`env python3`・`/usr/bin/env bash`・`sudo sh`・`xargs -0 rm`・
  `tee /etc/app.conf`・`zsh`・`ruby`・`sqlite3 db`・`ssh host bash -s`・`patch -p1`・`executable="/bin/bash"`・`uv run python -`）。
  以前の「FS_WRITE(content) 相当の情報行」は**子プロセスがデータで何をしうるかの保守的な上界**として正しかった。**取り消す**
  （コードを戻す）。v3 の誤 4（helm / kubectl への `communicate(input=None)`）は既知の限界に戻す（O41）。
- **G2 は健全でない**: 値の形（Seq / Map / Str / 文字列の定数）は型の証明ではない。未知の受け手への `strip()` などの TRANSFER
  の結果（`DOCS[name].strip().save()`）、`+` の結果、`self.remote = ""` / 大域の `SINK = ""` の後で木の中のオブジェクトに
  束縛し直す形、閉包・`global`・`builtins.list =`・`for list in` による `list` の束縛し直しで、本当の効果を消した（誤 clear）。
  **取り消す**（コードを戻す）。v3 の誤 2（`bytearray.append`）は既知の限界に戻す（O41）。
- **G3 の穴**: 再公開する `__init__.py` で同じ名前が複数回束縛される形（try / except の予備の import、条件つき import、import の後の
  代入）、`__init__.py` の中の絶対 import を相対として解いた形、読む側で呼ぶ名前を書き換える形（`global g; g = danger`、
  モジュール水準の `g = danger`）で、誤った解決・誤 clear を作った。**次の条件で残す**:
  1. 再公開をたどるのは、再公開するモジュールでその名前の束縛が**ちょうど 1 つ**で、それが**モジュール直下の
     `from ... import`** であり、関数の中で書き換えられない（`_scan_module_writes` に無い）ときだけ。
  2. たどる先は、その `ImportFrom` の `level` で解く（`level > 0` は相対、`__init__.py` の基準はパッケージ自身。`level == 0` は
     絶対 import として厳密に解く）。import 表（相対の点を捨てる）は使わない。
  3. 読む側で呼ぶ名前（別名つきなら別名）が、読む側のモジュールで 2 回以上束縛されるか、関数の中で書き換えられる
     （`_scan_module_writes`）なら、候補を `ambiguous` にする（全候補へ降りて opaque。別名の有無によらない一般の規則）。
- **G4 の穴（誤警報）**: 注釈で付けた外部の型（`requests.Session`）が、受け手の型の絞り込みで**末尾の名前だけ**で木の中の同名
  クラス（`Session`）に結ばれた。**条件**: 受け手の型のうち、モジュール部分が木の中に無い dotted 名（外部の型）は、木の中の
  クラスへの絞り込み（`_resolve_in_tree` の `typed_classes`）と継承の家族（`_class_family`）に使わない。外部の型の
  オブジェクトが木の中の同名クラスのインスタンスであることはない。この条件は D50 の DB の型にも同じくかかる（一般の規則）。
- G5（判定を変えない列）は変えない。

**D61 の改訂の追記**（改訂版を上のレビューの再現の木すべてで 1100319 と比べて見つけた）: 注釈の名前を import 表だけで解くと、
(a) `from httpx import AsyncClient` の後で同じモジュールが `class AsyncClient` を定義し直す形、(b) `from . import httpx`
（木の中のモジュール。import 表は相対の点を捨てる）の形で、外部の型と取り違える（どちらも不の行が 1 本増える誤警報。D50 の DB の型にも
同じくかかる）。**条件**: 注釈の先頭の名前がモジュール直下で 2 回以上束縛されるとき、またはその束縛が相対 import（`level > 0`）か
木の中のモジュールの import のときは、注釈で型を付けない。モジュール直下に束縛が無い（関数の中の import）ときは今までどおり。

### D61 の結果（2026-09-24、`scan_v2_run20`。解析器 `94c40b4`）と最終凍結

**v2（開発用）run18 → run20**: 効果 8,254 → 8,337（**消えた 0**、増えた 83）、CONTRADICTION（ユニット × site × kind）189 → 191
（**消えた 0**、増えた 2）、**D1+D2 の矛 151 → 153**（D1 107 → 109、D2 44 → 44）、D3 矛 39、D4 矛 0、両側比較 7/8 / 7/8 / 8/8 のまま。
増えた CONTRADICTION 2 件（bbwatch `list_courses` の `os.chmod` / `os.replace`）は、関数の中の別名つき import
（`from .cli import _authed as authed`）を G3 で解けるようになって届いたもので、原ソースで設定ディレクトリの chmod とセッションの
クッキーの保存を確かめた（readOnlyHint との矛、正しい）。G5 の列: 解決できなかった木の中の呼び出しを持つユニット 1,510、呼び出し 16,047。

**v3（回帰の確認だけ。改善として報告しない）v3_run1 → v3_run2**: CONTRADICTION の**消えた 0**。消えた効果 22 件はすべて teman2 の
`flowkit.py:53` で、`from nuzantara_mcp.workspace_flowkit import run as _run_flowkit_cli` の別名 `_run_flowkit_cli` を別モジュールの
同名の関数に末尾名で結んでいた**誤った経路**（G3 で正しい `workspace_flowkit.run` に解決し、その先の子プロセスの起動が出た）。

**D61 の経過**: 最初の実装（`672f0f0`）の G1・G2 は敵対的レビューで一般に正しくないと分かり取り消した。G3・G4 は健全性の条件を
足した（改訂 `a0d9304`・追記 `155aa7d`、実装 `94c40b4`）。レビューの再現の木 60 余りで D61 前との違いはすべて改善。

**最終凍結**: D61 の結果をもって解析器を凍結する（`docs/fingerprint.json`、タグ `analyzer-freeze-2`。**タグは学生が打つ**）。
これ以降、学生の最終評価（v2・v3 を含まない別データ）までは解析器を変えない。変えるなら `docs/preregistration.md` に逸脱として書く。
