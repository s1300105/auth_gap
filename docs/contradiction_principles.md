...
### 9.4 SQL の値の判定の順序（9.3 の補足。テストを書く前に見つけた抜け）

接頭辞の先頭語だけを見ると、`"SELECT * FROM t WHERE a = " + x` で `x` をモデルが選べるとき、
**文を重ねる注入**（psycopg などは 1 回の `execute` で複数の文を実行する）で `DELETE` が書けるのに
「読み取り」になる（原理 3-a に反する誤 clear）。そこで SQL の値は次の順で判定する:
