...
| 10 | 2026-09-21 | **解析器の変更（val エンジンの受け手型の規則、D35）を母集団 v2 の結果を見た後に入れた** | 解析器 235ae1e（run1 / run2 / scan run3） | 解析器 38531e7。F0a（`f0a_v2_run3`）と full scan（`scan_v2_run4`）を取り直し、**変更前後の両方を報告する** | 動機は v2 の結果ではなく §2.6 の受け入れ fixture F6/F7/F8（仕様が月 3 に定めた負例）の未達で、いずれも効果行が出ない false-clean 方向。規則は fixture と敵対的レビューの反証木で決め、v2 の数値を見て調整していない | 変更前: validator 保有 3.51%（主分母）、CONTRADICTION 72 件、r_D 53.3% |
| 11 | 2026-09-21 | **§3 の統一主張（3 座標の交差行）と、等級づけによる脆弱版 / 修正版の判別を主張から降ろす**（D36、O12 の案 (β)） | §3 の交差行 >= 3（3 プロジェクト由来）で統一を主張。§6 T1 の判別を主結果にする | 主張は「宣言 D と実効 M の照合」1 本。等級づけは道具として報告 | 母集団 v2 で交差行の候補 ~~5 行・2 プロジェクト~~（条件は 3 行・3 プロジェクト）。**訂正（D46、2026-09-23）: 正しくは 1 行・1 プロジェクト。**`scripts/intersection_rows.py` が §3 の `OPAQUE` 行の除外を実装していなかった。**D36 の結論（条件を満たさないので統一主張を降ろす）は変わらず、むしろ条件からさらに遠い。引用値のみ訂正する。**trig が traced のユニットは 14/2,231 = 0.6% で、§3 の「20% 未満なら SELECT を構造的主張に格下げする（撤回しない）」条項が発火する。MCP サーバでは dispatch がフレームワーク内にあり構造的に直らない | 交差行 5 行 / 2 プロジェクト、traced 0.6%、CONTRADICTION 79 件（16 木） |
| 12 | 2026-09-22 | **解析器の変更 2 件（D43: `Path` の祖先の受け手型 / D44: 解決できない呼び出しの主体）を母集団 v2 の結果を見た後に入れた** | 解析器 `b664e2bd`（`scan_v2_run4`） | 解析器 `229f5a29`（`scan_v2_run5`）。**full scan を取り直し、変更前後の両方を報告する**（`docs/scan_v2_run5_diff.md`） | どちらも動機は v2 の数値ではない。D43 は**先行研究 HintLint との突き合わせ**（D40）で見つかった誤 clear で、`readOnlyHint: true` のツールの `mkdir` が効果行を 1 本も出していなかった。D44 は D43 の敵対的レビューの副産物で、`_ev_Call` の fallback が root を引き継ぎながら主体を `Prin.OP` に固定していた**内部矛盾**である。規則は最小再現と敵対的レビューの反証木で決め、v2 の数値を見て調整していない。較正対 14 木で**消えた効果行 0**（D43 で +13、D44 で slot 変化 68 が全部 `OP → MODEL`）、`two_sided` は 8/8 と verdict-clearing が変更前と同一 | **変更前（run4）**: ユニット 2,231 / 効果 5,906 / CONTRADICTION 79（ユニット×site×kind）/ `GAP_INJECT` 一意 363 / 「root はあるのに主体が OP」2,053（21.6%）/ §3 の交差行 5 行・2 プロジェクト。**変更後（run5）**: ユニット 2,231（不変）/ 効果 5,987 / CONTRADICTION 85 / `GAP_INJECT` 一意 733 / 「root はあるのに主体が OP」71（0.7%）/ §3 の交差行 ~~15 行・5 プロジェクト~~ **→ 訂正（D46、2026-09-23）: 1 行・1 プロジェクト**。`scripts/intersection_rows.py` が §3 の「`OPAQUE` 行は数えない」を実装しておらず、`UNKNOWN` を併せ持つ行まで数えていた。**除外を適用すると run4 も run5 も 1 行・1 プロジェクトで、この解析器の変更は交差行をまったく動かしていない。**run4 の真の値も ~~5 行・2 プロジェクト~~ → **1 行・1 プロジェクト**。**消えたユニット 0、消えた CONTRADICTION 0、消えた `GAP_INJECT` 0** |
| 13 | 2026-09-23 | **`D_malformed`（snake_case の注釈）の記録を実装した**（仕様書 322 行目。D47 の点検で未実装と判明、D48 で修正） | `DKind.malformed` を設定する箇所が無く**構造的に常に空**。母集団 v2 の manifest で 0 件 | `Unit.malformed_fields` を足し、デコレータ経路・join した `Tool(...)` リテラル・低レベル経路の3 経路から `DKind.malformed` に合流する。**上界にも `explicit` にも入れない**（verdict は不変） | 仕様書が定めた報告項目が実装されていなかった。**規則の変更ではなく、書いてある規則の実装である。**snake_case を上界に効かせると「書いただけで宣言したことになる」= protocol に届いていないものを届いたものとして扱う誤 clear になるので、記録だけにした | **変更前（run5）**: `malformed` を持つユニット 0。**変更後（run6）**: 130 / 2,231 = 5.8%（9 木 / 87）。うち有効な宣言が 1 つも無いもの 124。キー別 `read_only_hint` 84 / `destructive_hint` 66 / `idempotent_hint` 54 / `open_world_hint` 14。**綴りが正しければ CONTRADICTION だったユニット 2 件**（目視確認済み。85 → 87 になりうる）。**verdict の数値は run5 と完全に一致**（ユニット 2,231 / 効果 5,987 / CONTRADICTION 85 / `GAP_INJECT` 1,928）。**`r_malformed` の分母は未決**（全ユニット 5.8% / 注釈を書いたユニット 8.1%。O27） |
| 14 | 2026-09-23 | **`db_unresolved`（受け手型を解決できない `.execute()`）の記録を実装した**（仕様書 1118 行目。D47 の点検で未実装と判明、D49 で修正） | 記録が無く、`effect_fp_audit.db_only_non_db_execute` は**到達不能な分岐**を数えていて構造的に常に 0。型が分からない `.execute()` は**痕跡なく消えていた** | `EffectExtractor.on_call` に `_note_db_unresolved` を足し `(method, relpath, lineno, receiver_classes)` を記録する。**効果は作らない**（仕様の「危険効果に数えない」を維持）。分子を `len(u.db_unresolved)` にした | 仕様書が定めた FP 監査の分子が実装されていなかった。**規則の変更ではなく、書いてある規則の実装である。** | **変更前（run6）**: `db_unresolved` 0 件（記録する仕組みが無い）。**変更後（run7）**: **2,078 件 / 482 ユニット / 19 木**（一意な位置 332）。DB 効果 230 件に対して**未解決率 90.0%**。受け手の型は 2,066 件が不明。**仕様が求める 20 件の抜き取りで 15 件（75%）が本物の SQL 呼び出しだった** → **O29（DB 効果の誤 clear）**。**verdict は run6 と完全に一致**（ユニット 2,231 / 効果 5,987 / CONTRADICTION 85 / `GAP_INJECT` 1,928） |
| 15 | 2026-09-23 | **DB 効果の受け手型の解決を広げた**（O29 / D50）: 呼び出し形の型遷移 `CALL_TYPE_TRANSITIONS`、psycopg / psycopg2 / psycopg_pool の CTOR、sink の DB 行に `AsyncSession` / `AsyncConnection` / `psycopg.Connection`（独立した行）、仮引数の型注釈 | 解析器 `5fc32c1`（`scan_v2_run7`）。`ATTR_TYPE_TRANSITIONS` の DB の 3 行はメソッドなのに属性でしか引かれず一度も効いていなかった。psycopg の接続を作る規則が無かった。型注釈は使わなかった | 解析器 `f248bfd`（`scan_v2_run9`）。**full scan を取り直し、変更前後の両方を報告する**（`docs/scan_v2_run9_diff.md`）。`scan_v2_run8` は site 名が付け替わった中間版で、比較の基準に使わない | 動機は D49 で記録を実装した `db_unresolved` の抜き取り（20 件中 15 件が本物の SQL）で、**誤 clear の修正**である。規則は受け手の書き方の分類（332 か所）と反例 5 つを含むテストで決め、解決率の数値を見て調整していない。解決率を上げる変更なので反例を正例と同じ重さで固定した | **変更前（run7）**: DB 効果 230 / `db_unresolved` 2,078（一意 332）/ **未解決率 90.0%** / 危険ユニット 1,187 / `GAP_INJECT` 一意 733 / CONTRADICTION 85。**変更後（run9）**: DB 効果 1,705 / `db_unresolved` 605（一意 205）/ **未解決率 26.2%** / 危険ユニット 1,492 / `GAP_INJECT` 一意 972 / CONTRADICTION 85。注釈で型を与えたユニット 377。**消えたユニット 0 / 消えた CONTRADICTION 0 / 消えた `GAP_INJECT` 0**。新規 DB 効果の抜き取り 20/20 が本物。残りの抜き取り 20 件は本物の SQL 14 / Google API 6 |
| 16 | 2026-09-23 | **受け手の型の解決（その 2）**（D51）: `Cls(...).m()` を受け手の型で解決する / `_ev_Yield` を足し `@contextmanager` の yield を戻り値にする / 戻り値の型注釈 | 解析器 `f248bfd`（`scan_v2_run9`）。`Cls(...).m()` は受け手の型が分かっていても降りず、yield の式は評価されず、戻り値の注釈は使われなかった | 解析器 `7078829`（`scan_v2_run10`、`docs/scan_v2_run10_diff.md`） | 最小再現で見つけた誤 clear の修正。**修正 1 は DB に限らない**（`Store("x").run(q)` の中の `os.system(q)` まで落ちていた）。規則は反例 5 つを含むテストで決めた | **変更前（run9）**: DB 効果 1,705 / 未解決率 26.2% / `GAP_INJECT` 一意 972 / SPAWN 123。**変更後（run10）**: DB 効果 1,741 / **未解決率 24.6%** / `GAP_INJECT` 一意 994 / SPAWN 127。CONTRADICTION 85（不変）。**消えたユニット・CONTRADICTION・`GAP_INJECT`・効果はすべて 0**。DB 以外の新しい効果 5 件はすべて目視で本物。**残りの 92.4% は深さの上限**（`MAX_DEPTH = 3` を変えるかは O29 で学生の判断待ち） |
| 17 | 2026-09-24 | **下向きの呼び出し深さ `MAX_DEPTH` を 3 → 4**（仕様書 243 行目「深さ 3」。D53 / D54、学生の決定） | `MAX_DEPTH = 3`（解析指紋の `caps`）。解析器 `ff04043`（`scan_v2_run10`） | `MAX_DEPTH = 4`（`scan_v2_run11`、`docs/scan_v2_run11_diff.md`）。深さ 3 の値は `Options(max_depth=3)` で出し直せる | O29 の残り（`db_unresolved` 569 件）の 92.4% が深さ 3 の上限に当たるユニットの中にあった。**全木の感度分析（深さ 3 / 4 / 5）を先に取り、結果を見てから決めた**（したがって純粋な事前登録ではない）。DCIChecker（k=3）と並べる行は深さ 3 でも出して併記する | **変更前（run10、深さ 3）**: 効果 7,503 / DB 効果 1,741 / **未解決率 24.6%** / FS_WRITE 326 / `GAP_INJECT` 一意 994 / **CONTRADICTION 85**。**変更後（run11、深さ 4）**: 効果 11,500 / DB 効果 2,490 / **未解決率 17.6%** / FS_WRITE 2,340 / `GAP_INJECT` 一意 1,108 / **CONTRADICTION 146**。**消えたユニット・効果・`GAP_INJECT`・CONTRADICTION はすべて 0。** 増えた CONTRADICTION 61 件の手検証: 本物 35 / 遠隔次第 13 / 意味は非破壊 8 / 誤検出 5（O30 / O31）。時間は 86 木で 396 → 424 秒。較正: two_sided 8/8（厳密 0/8・INJECT 1/8）不変、diff_effects で消えた行 0 |
| 18 | 2026-09-24 | **DB の書き込みを CONTRADICTION に数える**（表 D41 の DB 行の実装漏れ。D55） | 表は readOnly × データ変更・destructive=false × `UPDATE` / `DELETE` / `DROP` を矛盾と決めていたが、`contradiction()` と verdict が EXEC / SPAWN / FS_WRITE しか見ていなかった（解析器 `0644e39`、`scan_v2_run11`） | `sql_head` で判定（`SQL_MODIFY_HEADS` / `SQL_DESTRUCTIVE_HEADS`）。`PRAGMA` など（O23 #1）と読めない SQL は報告しない（`scan_v2_run12`、`docs/scan_v2_run12_diff.md`） | 決めたルールに実装を合わせる修正（新しい決定ではない）。**結果を見た後に見つけた**（O23 の材料づくりで `docs/o23_cells.md` を作ったとき） | **変更前（run11）**: CONTRADICTION 146。**変更後（run12）**: **178（+32、消えた 0）**。ユニット・効果・他の判定の行は不変。増えた 32 件は原典で確かめ、表の規則どおり（teamplay-talk 30 / asquared-mcp 2） |
| 19 | 2026-09-24 | **矛盾の判定を原理から導いた表で行う**（O23。D56、`docs/contradiction_principles.md` §6 / §7） | D32 / D41 / D55 の規則: CONTRADICTION は D1 / D2 × EXEC / SPAWN（全件）/ FS_WRITE / DB。O23 の 5 マスは未決で報告しない（`scan_v2_run12`） | 学生が選んだ原理（1-i b / 1-ii b / 2 a / 3 a / 4 a）から機械的に導いた §7 の表。不明は注記として別に数える。D3 / D4 は探索的な別列（`scan_v2_run13`） | 件数を見て 1 マスずつ決めないため、原理を先に選んでコミットし、表とテストを実装より先にコミットした。**ただし表を書いた者は run11 の件数を見ていた**（純粋な事前登録ではない）。判定者は 1 人 | **変更前（run12）**: CONTRADICTION 178（すべて D1 / D2）。**変更後（run13）**: 220（D1 矛 109 / D2 矛 56 / **D1 + D2 = 165** / D3 矛 51 / D4 矛 5。不明 D1 86 / D2 95 / D3 81 / D4 77）。増えた 80 / 消えた 38（消えた 38 はすべて原理の分岐で説明がつく）。感度分析: 主指標は原理を 1 つ反対側にしても 153〜165、原理 2 を b にしたときだけ 346 |
| 20 | 2026-09-24 | **O30 / O32 / O33 の修正と §9.5**（D57、`docs/contradiction_principles.md` §9） | 末尾名の解決がテストファイルの定義に結びつく / 相対 URL の値全体を `url.host` に入れる / 原理 3-a を主体 MODEL だけで判定 / 属性の呼び出しをクラス外の関数へ確度 resolved で結ぶ（`scan_v2_run13`） | テストファイルの定義を本体からの末尾名の候補にしない / 相対 URL は `url.path` と `base_url` の宛先 / 3-a は MODEL かつ resolved、SQL は §9.4 の順序 / クラス外の関数への末尾名は by_name（`scan_v2_run14`） | run13 の手検証（D56）で見つけた誤検出の原因を、原理から導いた規則（実装より先にコミット）で直した。§9.5 は修正の敵対的レビューで見つけた。結果を見た後の変更 | **変更前（run13）**: 効果 11,500 / 危険なユニット 1,593 / CONTRADICTION 220 / **D1 + D2 の矛 165** / D3 矛 51 / D4 矛 5 / GAP_INJECT の行 2,760。**変更後（run14）**: 10,193 / 1,346 / 191 / **154** / 38 / 0 / 2,810。消えた効果 1,010 はすべて xagent のテストファイル経由、消えた CONTRADICTION 29 はすべて §9 の規則で説明がつく。**O30 が既存のゲートの誤 clear（O34）を 1 件表に出した**（canvas-mcp の `body`） |
| 21 | 2026-09-24 | **O31 / O34 の修正**（D58、規則 `6da93f6` / A2 の扱い `0c36cbb` / O31 の改訂 `82e3258`） | 呼び出し先の `if` は両方の枝を実行し、`return` / `raise` の後も実行する / ゲートの主語一致はどれか 1 つの根（`scan_v2_run14`） | 静的な仮引数（直接の呼び出しで、リテラルの既定値・リテラルの実引数・呼び出し元で静的な名前から来て、束縛し直さないもの）で決まる `if` は取られる枝だけ、終わる文の後は実行しない / 主語一致は値の MODEL の根すべて（`scan_v2_run16`） | 凍結前に直すと学生が決めた（run13 / run14 の手検証で見つけた誤検出と誤 clear）。**O31 の最初の実装（run15）は敵対的レビューで誤 clear 23 行が見つかり、規則を狭めて改訂した。** 結果を見た後の変更 | **変更前（run14）**: 効果 10,193 / CONTRADICTION 191 / **D1 + D2 の矛 154** / D3 矛 38 / D4 矛 0 / GAP_INJECT の行 2,810 / 両側比較 事前登録照合 8/8。**変更後（run16）**: 7,915 / 189 / **151** / 39 / 0 / 1,705 / **7/8**（A2 が落ちる。これまでの 8/8 は O34 の誤 clear に一部支えられていた。規則は変えずに報告する。O35）。消えた効果 2,346 件はすべて敵対的レビューで正しいと確かめた集合に入る |
| 22 | 2026-09-24 | **O36 / O37 の修正と HTTP の宛先の `url=`**（D59、規則 `693ee79` と追記 `5ab87cd`） | 単項演算が被演算子の値のまま / Atom の合流が none と定数の型を比べない / 関数本体の import が束縛しない / except 節の入口が try 前の env だけ / HTTP の body が data= か json= の片方 / url.host が位置だけ（`scan_v2_run16`） | 演算の結果 / 可換な合流 / import で束縛 / try 前と try 本体の各文の後の合流 / 仕様書 229 行目の `data|json` / 位置に無ければ `url=`（`scan_v2_run18`） | run15 の敵対的レビューで見つけた値の欠陥（O36）と、仕様書からのずれ（O37）。url= は run17 の突き合わせで見つけた同じ種類のずれ。結果を見た後の変更。**D59 を最後に解析器を凍結した**（`docs/fingerprint.json`） | **変更前（run16）**: 効果 7,915 / CONTRADICTION 189 / **D1 + D2 の矛 151** / D3 矛 39 / D4 矛 0 / GAP_INJECT の行 1,705 / 両側比較 7/8。**変更後（run18）**: 8,254 / 189 / **151** / 39 / 0 / 2,290 / 7/8。消えた効果・CONTRADICTION・ユニットは 0。宛先の無い NET 効果 1,693 → 6 |
| 23 | 2026-09-24 | **v3（開発用の確認）の所見のうち一般に正しい修正だけを入れた**（D60 / D61。G3 再公開と別名つき import・G4 注釈による proxy の受け手の型付け・G5 解決できなかった呼び出しの列） | v3 で見つけた誤と見落とし（`scan_v2_run18`） | G1（標準入力）と G2（組込み型の受け手）は敵対的レビューで一般に正しくないと分かり取り消した。G3 / G4 は健全性の条件つき（`scan_v2_run20`） | 学生の方針「検証データに合わせて直さない。一般的に役立つものだけ直す」（D61）。v3 は開発用で、v3 の数値の改善は報告しない。学生の最終評価は v2 / v3 を含まない別データ | **変更前（run18）**: 効果 8,254 / CONTRADICTION 189 / **D1 + D2 の矛 151** / D3 矛 39 / D4 矛 0 / 両側比較 7/8。**変更後（run20）**: 8,337 / 191 / **153** / 39 / 0 / 7/8。消えた効果・CONTRADICTION・ユニットは 0。**これをもって最終凍結**（`analyzer-freeze-2`） |
