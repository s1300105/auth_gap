...
**0. 主張は 1 本（D36）。** 「宣言 D と実効 M の照合」だけを主軸にし、3 座標の統合と
等級づけによる脆弱版 / 修正版の判別は主張から降ろした。この母集団では交差行が
5 行 / 2 プロジェクト（§3 の条件は 3 行 / 3 プロジェクト）、trig が traced のユニットは
14/2,231 = 0.6%（§3 の格下げ条項の閾値は 20%）で、統一の証拠は事前登録の条件を満たさない。
