...
#: **仕様書 §（243 行目）は 3。** 2026-09-24 に学生の決定で 4 に上げた（D53 の感度分析 /
#: D54、`docs/preregistration.md` の逸脱 #17）。深さ 3 の値は `Options(max_depth=3)` で出し直せる
#: （DCIChecker の k=3 と条件を揃える比較はそれで出す）。
MAX_DEPTH = 4
